...
from negative_generation import gen_all_negatives, gen_random_negatives
...
def extract_customer_features(df):
    ...
    customer_price_stats.rename(columns={"mean": "item_avg_price", "std": "item_std_price"}, inplace=True)
    most_purchased_category = group["item_category"].agg(lambda x: x.mode()[0])

    # Features de comportamiento y preferencia
    ...
    customer_preferred_collection = group["item_collection_type"].agg(lambda x: x.mode()[0] if len(x.mode()) > 0 else "Unknown")
    ...
    customer_avg_days_from_release = (group["purchase_timestamp"].max() - group["item_release_date"].max()).dt.days
    customer_prefers_new_items = (customer_avg_days_from_release < 30).astype(int)

    # Features de navegación y rating
    ...
    # Features técnicas
    customer_preferred_device = group["purchase_device"].agg(lambda x: x.mode()[0] if len(x.mode()) > 0 else "unknown")
...
def build_processor(df, numerical_features, categorical_features, free_text_features, training=True):
    ...
    if training:
        ...
        free_text_transformers = [(col, CountVectorizer(), col) for col in free_text_features]
    ...
    other_cols = [c for c in df.columns if c not in numerical_features + categorical_features + free_text_features]
...
def preprocess(raw_df, training=False):
    ...
    raw_df["customer_cat_is_prefered"] = (raw_df["item_category"] == raw_df["customer_prefered_cat"])
    categorical_features = ["customer_gender", "item_category", "item_img_filename"]
    free_text_features = ["item_title"]

    processed_df = build_processor(raw_df, numerical_feat, categorical_features, free_text_features, training=training)
...
def read_train_data():
    ...
    customer_feat_cols = [col for col in all_columns if "customer" in col]
    unique_customers = processed_pos[customer_feat_cols].drop_duplicates(subset=["customer_id"])
    ...
    processed_full = pd.concat([processed_pos, processed_neg], axis=0).sample(frac=1).reset_index(drop=True)
